...
wind();
var main = document.getElementById("IndexMain");
fetch('news.json').then(function (response) {
  return response.json();
}).then(function appendNews(json) {
  for (var i = json.length - 1; i >= 0; i = i - 1) {
  ...
  var photoMain = document.getElementsByClassName("mainImage");

  var _loop = function _loop(_i) {
    photoMain[_i].addEventListener("click", function () {
      openPicture(photoMain, _i);
    });
  };

  for (var _i = 0; _i < photoMain.length; _i++) {
    _loop(_i);
  }
});
var open = false;
var dropdown = document.getElementById("linkDown");
var downContent = document.getElementById("dropdown-content");
...
dropdown.addEventListener("click", function () {
  if (open) {
    ...
    window.setTimeout(function () {
      downContent.style.display = "grid";
    }, 200);
  }
...
function openPicture(pic, ind) {
  var newWhole = document.createElement("div");
  newWhole.className = "openImg";
  var newInner = "<div class=\"exitBtn\" id=\"exitBtn\">\u2716</div>" + pic[ind].innerHTML;
